  ...
  return (
    <motion.div
      ...
      <div className="p-4">
        ...
        <p className="font-bold mb-2">Category : <span className="text-sm"> {book_category}</span> </p>
        <p className="text-sm line-clamp-2">{book_overview}</p>
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm">{total_page} pages</span>
          <span className="text-sm font-medium text-bgBtn">
            {upvote} Upvotes
          </span>
        </div>
          </div>
          <div className="">
              <Link to={`/books/${book._id}`} className="flex flex-grow items-end"><button className="btn bg-bgBtn text-lg hover:bg-hoverBtn text-textBtn w-full">Details</button></Link>
          </div>
    </motion.div>
  );
};
